...
export default function Header({ content, locale }: { content: HeaderContent | null; locale: string }) {
  ...
  return (
    <>
      <header className={`sticky top-0 z-50 flex items-center justify-between px-6 md:px-12 pt-6 pb-2 transition-colors duration-300 ${scrolled ? "bg-white shadow-sm" : "bg-transparent"}`}>
        ...
        <div className="relative z-60 flex items-center gap-3">
          <button
            ...
            className="flex items-center gap-2 bg-foreground text-white px-5 py-2.5 rounded-full text-sm font-medium hover:bg-card-dark transition-colors"
          >
            {menuOpen ? close : menu}
